...
const DashboardViewers = () => {
  ...
  const getDashboardViewers = async () => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_BACK_URL}/api/users/dashboard-viewers`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
          },
      ...
      setDashboardViewers(response.data.dashboardViewers);
    } catch (error) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setErrorBack(error.response?.data?.message || "Erreur lors de la création");
      setTimeout(() => setErrorBack(""), 2000);
    }
  };
  ...
  return (
    <div className="bg-gray-100 min-h-screen p-6 content">
      <div className="mb-6">
        ...
        <p className="text-gray-600">
          Visualisation et gestion des utilisateurs ayant accès au tableau de bord
        </p>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-lg overflow-hidden">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-gray-700">
            <thead className="bg-gray-50">
              <tr>
                <th className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Prénom</th>
